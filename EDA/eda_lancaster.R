...
# Clientes e vendas com 0 clientes num dia
...
par(mfrow = c(1, 1)) # Outliers não foram removidos porque correspondem a eventos reais



# Correlação entre variáveis da loja
numeric_vars <- df %>%
...
# Grafico Sales vs Num_Costumers
...
# Série temporal
# Ver tendência, picos e padrões da loja de Baltimore
...
ggplot(df, aes(DayOfWeek, Num_Customers)) +
  ...
  theme_minimal() # Verificamos que DayOfWeek é um preditor relevante
...
# Consistência do padrão semanal entre anos
df %>%
...
# Eventos turísticos e o respetivo impacto
