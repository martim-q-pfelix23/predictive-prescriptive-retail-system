# Forescasting Univariado - MLPE + RF
library(forecast)
library(rminer)
...
# MLPE - Rede neuronal (multilayer perceptron ensemble)
set.seed(42)
...
# Random Forest 
# Robusto a outliers extremos identificados na EDA
# Lida bem com a distribuição assimétrica de Num_Customers
set.seed(42)
...
# Dois conjuntos de lags para comparação
timelags_17   <- c(1, 7)
...
W   <- (L - H) - (RUNS - 1) * S_step
W2  <- W - max(timelags_17)    # para D  com c(1,7)
W2b <- W - max(timelags_1728)  # para D2 com c(1,7,28)
...
# Todos os MLPE
# Gráfico comparativo dos modelos MLPE - Todos os Lags
...
# Gráfico comparativo dos modelos RF - Todos os Lags
old_par <- par(no.readonly = TRUE)
par(oma = c(5, 0, 0, 0), mar = c(4, 4, 4, 2))
