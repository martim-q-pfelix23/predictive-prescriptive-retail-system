# Forescasting Univariado - NNETAR + ETS
library(forecast)
library(rminer)
...
# NNETAR
# Lida melhor com outliers extremos
set.seed(42)
...
plot_with_legend(
  ...
  store_name,
  YR,
  legend_labels = c("target", "NNETAR pred.")
)


# ETS
# ets() seleciona automaticamente o melhor modelo da família
ETS      <- ets(TR_ts)
print(ETS)
...
plot_with_legend(
  ...
  store_name,
  YR,
  legend_labels = c("target", "ETS pred.")
)
