...
store_file <- "philadelphia_clean.csv"
...
r_mlpe <- plot_with_legend("MLPE", Y, Pred_mlpe, store_name, YR)

# Random Forest
set.seed(42)
M_rf    <- fit(y~., D[hd$tr,], model = "randomForest")
Pred_rf <- lforecast(M_rf, D, start = hd$ts[1], horizon = H)
r_rf   <- plot_with_legend("RandomForest", Y, Pred_rf, store_name, YR)
...
plot_gw_legend(all_Y, list(all_mlpe, all_rf),
               c("MLPE", "RandomForest"), c("blue", "red"),
               store_name, "MLPE + RF | lags c(1,7)")

plot_gw_legend(all_Y, list(all_mlpe_28, all_rf_28),
               c("MLPE", "RandomForest"), c("blue", "red"),
               store_name, "MLPE + RF | lags c(1,7,28)")

plot_gw_legend(all_Y, list(all_mlpe_1237, all_rf_1237),
               c("MLPE", "RandomForest"), c("blue", "red"),
               store_name, "MLPE + RF | lags c(1,2,3,7)")

plot_gw_legend(all_Y, list(all_mlpe_137, all_rf_137),
               c("MLPE", "RandomForest"), c("blue", "red"),
               store_name, "MLPE + RF | lags c(1,3,7,14,21,28)")
...
n <- length(all_Y)
ylim_all <- range(c(all_Y, all_mlpe, all_mlpe_28, all_mlpe_1237, all_mlpe_137))

plot(1:n, all_Y, type="l", lwd=2, col="black",
     ylim = ylim_all,
...
legend("bottom",
       legend = c("Real", "c(1,7)", "c(1,7,28)", "c(1,2,3,7)", "c(1,3,7,14,21,28)"),
       col    = c("black", "red", "green", "blue", "purple"),
       lty    = c(1, 2, 2, 2, 2),
       lwd    = 2, horiz = TRUE, bty = "n", cex = 0.75)
...
ylim_all <- range(c(all_Y, all_rf, all_rf_28, all_rf_1237, all_rf_137))
